...
function getClientIp(req) {
  ...
  const forwardedIps = forwardedFor?.split(',').map((ip) => normalizeClientIp(ip)).filter(Boolean);
  const proxyReportedIp = forwardedIps?.[forwardedIps.length - 1];

  return proxyReportedIp
